...
sub MAIN(:$backends) {
    my Bool %backends = %(set $backends.?split: ",");
    %backends<parrot moar jvm>»++ if %backends<ALL> or !%backends;

    my $devnull     = IO::Spec.devnull;
    %backends<parrot> &&= shell("perl6-p -e 1 >$devnull 2>$devnull").status == 0;
    %backends<moar>   &&= shell("perl6-m -e 1 >$devnull 2>$devnull").status == 0;
    %backends<jvm>    &&= shell("perl6-j -e 1 >$devnull 2>$devnull").status == 0;
    ...
    $Makefile   ~= fill_template 'build/Makefile-Parrot.in' if %backends<parrot>;
    $Makefile   ~= fill_template 'build/Makefile-Moar.in'   if %backends<moar>;
    $Makefile   ~= fill_template 'build/Makefile-JVM.in'    if %backends<jvm>;
